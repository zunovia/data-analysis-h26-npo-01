...
出典：**公益社団法人及び公益財団法人の寄附金収入に関する実態調査**（内閣府）
テーマ：公益法人の寄附金収入の実態・税額控除制度・募金活動
調査対象：公益社団法人・公益財団法人（全法人）
有効回答数：約6,155法人
データ期間：平成26年度～平成30年度（2014～2018年度）
データ格納先：`data06/公益社団法人及び公益財団法人の寄附金収入に関する実態調査/`
...
| ファイル | サイズ | シート数 | 内容 |
|---------|--------|---------|------|
| `syukeidata_r1.xlsx` | 299KB | **92** | 図1～図86（問1～問19の集計・クロス集計グラフデータ） |
| `集計データ.xlsx` | 1,007KB | **9** | 単純集計・クロス集計の詳細表＋グラフ用元データ |
| `集計表.xlsx` | 51KB | **13** | サマリー集計表（回答法人数、寄附金収入額、募集方法等） |

### 3ファイルの関係

```
集計表.xlsx         ：最もコンパクトなサマリー（13テーマ）
  ↕ 同一データの詳細版
集計データ.xlsx     ：単純集計＋クロス集計の詳細表
  ↕ 図表用に再構成
syukeidata_r1.xlsx  ：92シートの図表データ（問1～19の全クロス集計）
```
...
| 問 | テーマ | シート例 |
|----|--------|---------|
| 問1 | 公益目的事業費用の額（規模区分） | 図1 |
| 問2 | 職員数（常勤・非常勤） | 図3～6 |
| 問3 | 寄附金収入金額（全体・個人・法人、5年推移） | 図7～15 |
| 問4 | 寄附金収入の必要性 | 図16～18 |
| 問5 | 寄附金必要性の理由 | 図19～22 |
| 問6 | 寄附金を得るための活動の有無 | 図23～26 |
| 問7 | 募金活動の具体的手段 | 図27～31 |
| 問8 | 寄附金が必要でない理由 | 図32～35 |
| 問9 | 税額控除対象法人の該当 | 図36～39 |
| 問10-11 | 証明取得日・PST要件の認識 | 図40～45 |
| 問12 | PST要件の充足状況 | 図48～51 |
| 問13 | 税額控除証明の取得希望 | 図52～56 |
| 問14-15 | 取得の支障・取得しない理由 | 図57～64 |
| 問16 | 寄附の受入形態（現金・現物） | 図69～72 |
| 問17 | みなし譲渡所得課税の非課税措置 | 図73～77 |
| 問18 | 税制改正の認知 | 図78～82 |
| 問19 | 非課税措置の適用件数 | 図83～86 |

### データ形式

- 各シートはヘッダーが**複数行にまたがる非整形Excel表** → `header=None` で読み込み
- 数値はn（法人数）と%（構成比）がペアで格納（奇数行=n、偶数行=%）
- クロス集計軸：公益目的事業費用規模（7区分）、職員数、寄附金額帯、税額控除該当有無、事業類型
- サンプルサイズは各行に埋め込まれている

### syukeidata_r1.xlsx シート構造パターン

- **単純集計シート**（図1,3,4,5,7～12,16,19,23,27,28,32,36,40,41,43,48,52,57,60,69,73,78,83）
  - R8前後にヘッダー行、R9にn行、R10に%行（計10行前後）
- **クロス集計シート**（図2,6,13～15,17～18,20～22,24～26,29～31,33～35,37～39,42,44～47,49～51,53～56,58～59,61～65,66-1～68-3,70～72,74～77,79～82,84～86）
  - ヘッダー行の後、公益目的事業費用規模（7区分）でクロス集計（計20～48行）
  - 全体行 + 規模別行がn/%ペアで並ぶ
...
| ファイル | 内容 |
|---------|------|
| `analysis_report_data06.ipynb` | 分析ノートブック |
| `data06/レポート/images/` | matplotlib生成チャート |
| `data06/レポート/report.md` | Markdown分析レポート |
| `data06/レポート/html_report/index.html` | HTML/CSS/JSインタラクティブレポート |

### 画像生成スクリプト（data06）

```
tmp/explore_data06.py               # 全ファイルの構造探索
tmp/explore_data06_summary.py       # 要約版探索
tmp/generate_images_data06.py       # チャート画像生成
tmp/generate_notebook_data06.py     # Notebook再生成
```
